...
The [PartitionedBlobUrlUsage](PartitionedBlobUrlUsage.md) policy controls whether Blob URLs are partitioned during fetching and navigation.
...
If storage partitioning is disabled for a given top-level origin either by [ThirdPartyStoragePartitioningBlockedForOrigins](ThirdPartyStoragePartitioningBlockedForOrigins.md) or [DefaultThirdPartyStoragePartitioningSetting](DefaultThirdPartyStoragePartitioningSetting.md), then Blob URLs aren't partitioned.

The policy is scheduled to be available through Microsoft Edge version 143. After this version, the policy will be removed, and Microsoft Edge will no longer support unpartitioned blob storage.
