...
```
  ...
  This policy is deprecated in favor of ChannelSearchKind, which has the same functionality, and becomes obsolete in 124 release. The default channel search order is WebView2 Runtime, Beta, Dev, and Canary.
...
This policy is obsolete because this policy was originally implemented as a safety measure if there was breakage, but none has been reported.
...
```
  ...
  Setting the policy specifies a list of hostnames or hostname patterns (such as '[\*.]example.com') that which doesn't upgrade to HTTPS. Organizations can use this policy to maintain access to servers that don't support HTTPS, without needing to disable "AutomaticHttpsDefault" or "HttpsUpgradesEnabled".
...
With Microsoft Edge 124, this
check is always enabled.

Microsoft Edge 123 and earlier have the
following behavior:

If this policy is set to be enabled,
...
future release, which enables this check by default. At that point, this
...
The policy is obsoleted starting from Microsoft Edge version 136,
but the key check is always enabled since Microsoft Edge version 124.
