...
| 134 | Enterprise users with Multifactor Authentication (MFA) and Conditional Access (CA) may not have Single Sign-on (SSO) capabilities with Microsoft 365 Copilot Chat. The user may see a "sign in" button when they try to use Copilot. |  |"The user may see a "sign in" button when they try to use Copilot." |
