...
| --- | --- | --- | --- |
| 133 | This issue is fixed in Microsoft Edge stable release 133.0.3065.69.  Note: For some users it still might be necessary to "sign out (keeping locally saved data)"and then "sign-in" again for sync functionality to resume.  | For sync functionality to resume, users can "sign out (keeping locally saved data)"and then "sign-in" again.  For AIP-protected PDF files, please use other tools such as the Azure Information Protection Viewer. | We're working to release a fix for Microsoft Edge as soon as possible. |
