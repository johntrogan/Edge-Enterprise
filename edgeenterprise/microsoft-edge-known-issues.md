...
| --- | --- | --- | --- |
| N/A | When Microsoft Edge is installed, it includes the *Microsoft.MicrosoftEdge.Stable* host package. This package is a browser host package and enables platform functionality that would otherwise be unavailable to the browser as a Win32 application (System Notifications, Taskbar/Start pinning with UWP-like features, etc.). It isn't recommended to remove it. <br><br>Customers may observe that the host package is an older version than the current browser version.  |  | This is expected behavior and doesn't indicate any security vulnerability. Additionally, it doesn't imply Edge nor any other component is out of date. The host package is updated independently when it is detected that the package is either in a bad state or a new feature is added to the host package. | 
| 137 | 	Starting in Microsoft Edge Stable version 137.0.3296.52, customers may encounter unexpected leading spaces in the browser window title, which can break automation workflow for non-English language environments.  |  | This issue is fixed in Microsoft Edge stable release 137.0.3296.58. |
| 136 | 	Starting in Microsoft Edge Stable version 136.0.3240.50, some text isn't displayed, and rendering is broken when viewing certain PDF files containing Japanese fonts. **Note:** This issue occurs when the system has fonts with Japanese file names installed. | This issue only occurs with the new Adobe Acrobat powered built-in PDF reader. Enterprise administrators can temporarily avoid the issue by disabling the [NewPDFReaderEnabled](/deployedge/microsoft-edge-browser-policies/newpdfreaderenabled) policy. Each user can also temporarily avoid the issue by disabling the New PDF Viewer in edge://flags/#edge-new-pdf-viewer. | This issue is fixed in Microsoft Edge stable release 136.0.3240.92. |
| 136 | 	On the Microsoft Edge New Tab Page (NTP), customers may see differing feeds.  Even when NTP policies are configured, users may have the Discover or My Feed selected.  |  | The NTP issue and Microsoft 365 admin center configuration issue have been fixed. |
| 136 | 	For users with the 'Auto hidden' sidebar visibility setting, their Outlook or Teams link may not be showing the sidepane content as expected. | | This issue is fixed in Microsoft Edge stable release 137.0.3296.52. |
| 136 | 	Microsoft Editor (spell checker) doesn’t offer suggestions for misspelled words | The issue was identified in Microsoft Editor. Users that need spell checking assistance, can rely on the Basic functionality. This can be enabled under edge://settings/languages or via Group Policy using the [MicrosoftEditorProofingEnabled](/deployedge/microsoft-edge-policies#microsofteditorproofingenabled) policy. | This issue is fixed in Microsoft Edge stable release 136.0.3240.64. |
...
| 134 | Starting in version 134, customers may encounter an issue that affects extensions that bring up a window. The extension may get paused in the background if the window is minimized or occluded if the extension is meant to keep running always. | Enterprise administrators can avoid the issue by disabling the Sleeping Tabs feature using the [SleepingTabsEnabled](/deployedge/microsoft-edge-policies#sleepingtabsenabled) policy, or by adding the extension path in the [SleepingTabsBlockedForUrls](/deployedge/microsoft-edge-policies#sleepingtabsblockedforurls). | This issue is fixed in Microsoft Edge stable release 136.0.3240.92.  Also, [see Edge v134 Sleeping Tabs feature affects the UiPath extensions 22.10, 23.4 and 23.10](https://forum.uipath.com/t/edge-v134-sleeping-tabs-feature-affects-the-uipath-extensions-22-10-23-4-and-23-10/2757081), for more information|
| 134 | Some customers may unexpectedly start seeing the sidepane open with context for their Teams and Outlook links. In Edge 134, a bug fix introduced a regression where if the sidebar was turned off (not visible), Edge would auto-open Teams or Outlook context in the sidepane. | Users can manually toggle off the auto-open context experience in Edge settings (Copilot and sidebar/App and notification settings/Teams + Outlook). | This issue is fixed in Microsoft Edge stable release 136.0.3240.50. |
