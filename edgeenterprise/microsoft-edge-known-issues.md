...
| --- | --- | --- | --- |
| 133 | Customers may not be able to start the sync functionality or open AIP-protected PDF files in Microsoft Edge. A bug in Microsoft Edge version 133 is causing excessive traffic to the MIP service, which has resulted in the temporary blocking of sync functionality and the inability to open AIP-protected PDF files in Microsoft Edge. | For sync functionality to resume, users can "sign out (keeping locally saved data)"and then "sign-in" again.  For AIP-protected PDF files, please use other tools such as the Azure Information Protection Viewer. | We're working to release a fix for Microsoft Edge as soon as possible. |
