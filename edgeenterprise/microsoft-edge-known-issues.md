...
| --- | --- | --- | --- |
| N/A | When Microsoft Edge is installed, it includes the Microsoft.MicrosoftEdge.Stable host package. This package is a browser host package and enables platform functionality that would otherwise be unavailable to the browser as a Win32 application (System Notifications, Taskbar/Start pinning with UWP-like features, etc). It is not recommended to remove it. Customers may observe that the host package is an older version than the current browser version.  |  | This is expected behavior and does not indicate any security vulnerability. Additionally, it does not imply Edge nor any other component is out of date. The host package is updated independently when it is detected that the package is either in a bad state or a new feature is added to the h | 
