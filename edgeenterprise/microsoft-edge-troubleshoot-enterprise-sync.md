...
1. Restart Microsoft Edge and go to *edge://sync-internals*. Look at the **Microsoft Entra account Key Status** section to see if any of the following messages are present:
   - MIP Result = "Success": This error means server data might be encrypted with a lost key. A data reset is needed to resume sync.
   - MIP Result = "No permissions": It's possibly caused by a Microsoft Entra ID change or tenant subscription changes. A data reset is needed to resume sync.
